...
PropData Properties[] = {
	...
	PropData_CheckBox	(PROPID_ROTATIONAA,		IDS_ROTATIONAA,			IDS_ROTATIONAA ),
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	if ( ps != NULL )		// Do the following if this surface exists
	{
		...
		cSurface icon;			// New surface variable for us to use
		icon.Create(32, 32, ps);	// Create a surface implementation from a prototype (frame editor win)
		icon.LoadImage(hInstLib, EXO_ICON, LI_REMAP);	// Load our bitmap from the resource,
														// and remap palette if necessary
		cSurface is;
		is.Create(edPtr->swidth, edPtr->sheight, ps);
		is.Fill(200,200,200);
		
		icon.Stretch(is,0,0,w,h,0,0,32,32,BMODE_OPAQUE,BOP_COPY,0,0);
...
switch (nPropID)
{
	case PROPID_ROTATIONAA:
		return new CPropDWordValue(edPtr->rotationAA);
}
...
void WINAPI DLLExport SetPropCheck(LPMV mV, LPEDATA edPtr, UINT nPropID, BOOL nCheck)
{
	...
	{
	case PROPID_ROTATIONAA:
		edPtr->rotationAA = nCheck;
		mvInvalidateObject(mV, edPtr);
		mvRefreshProp(mV, edPtr, PROPID_ROTATIONAA,0);
		break;
	}
