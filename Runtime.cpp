...
{
	rdPtr->rHo.hoImgWidth = max(edPtr->swidth,1);
	rdPtr->rHo.hoImgHeight = max(edPtr->sheight,1);

	rdPtr->FlipHoz = false;
	rdPtr->FlipVer = false;

	rdPtr->Angle = 0;
	rdPtr->RotationAA = false;

	rdPtr->SourceWidth = max(edPtr->swidth,1);
	rdPtr->SourceHeight = max(edPtr->swidth,1);

	rdPtr->SourceX = edPtr->sx;
	rdPtr->SourceY = edPtr->sy;

	rdPtr->RotationAA = edPtr->rotationAA;
...
short WINAPI DLLExport DisplayRunObject(LPRDATA rdPtr)
{
	...
	int boxWidth = rdPtr->SourceWidth;
	int boxHeight = rdPtr->SourceHeight;
	
	int centerX = boxWidth/2;
	int centerY = boxHeight/2;

	cSurface sourceImg;
	if ( GetSurfacePrototype(&pProto, ps->GetDepth(), ST_MEMORYWITHDC, SD_DIB) )
	{
		sourceImg.Create(rdPtr->SourceWidth,rdPtr->SourceHeight, pProto);	
		if( !sourceImg.IsValid() ) return -1;
	}
	...
	else
	{
		...
		boxWidth  = abs( (int)(cos(rdPtr->Angle)*rdPtr->SourceWidth) )  + abs( (int)(sin(rdPtr->Angle)*rdPtr->SourceHeight) );
		boxHeight = abs( (int)(cos(rdPtr->Angle)*rdPtr->SourceHeight) ) + abs( (int)(sin(rdPtr->Angle)*rdPtr->SourceWidth)  );
		centerX = boxWidth/2;
		centerY = boxHeight/2;
		...
		if ( GetSurfacePrototype(&pProto, ps->GetDepth(), ST_MEMORYWITHDC, SD_DIB) )
		{
			rotateImg.Create(boxWidth,boxHeight, pProto);	
			if( !rotateImg.IsValid() ) return -1;
		}
	
		ps->Blit( rotateImg,
			0,
			0,
			rdPtr->SourceX + rdPtr->SourceWidth/2 - centerX,
			rdPtr->SourceY + rdPtr->SourceHeight/2 - centerY,
			boxWidth,
			boxHeight,
			BMODE_OPAQUE,
			BOP_COPY
		...
		rotateImg.Rotate( sourceImg, (int)( rdPtr->Angle * ( 180.0/PI )) , rdPtr->RotationAA);

		rotateImg.Delete();
	}

	if(rdPtr->FlipHoz)
		sourceImg.ReverseX(0,0,rdPtr->SourceWidth,rdPtr->SourceHeight);
	
	if(rdPtr->FlipVer)
		sourceImg.ReverseY(0,0,rdPtr->SourceWidth,rdPtr->SourceHeight);
